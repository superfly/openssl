...
func go_write_bio_write(b *C.BIO, data *C.char, size C.int) (rc C.int) {
	defer func() {
		if err := recover(); err != nil {
			fmt.Printf("[crit] openssl: writeBioWrite panic'd: %v\n", err)
			rc = -1
		}
	}()
	ptr := loadWritePtr(b)
...
func go_write_bio_ctrl(b *C.BIO, cmd C.int, arg1 C.long, arg2 unsafe.Pointer) (
	...
	defer func() {
		if err := recover(); err != nil {
			fmt.Printf("[crit] openssl: writeBioCtrl panic'd: %v\n", err)
			rc = -1
		}
	}()
	switch cmd {
...
func go_read_bio_read(b *C.BIO, data *C.char, size C.int) (rc C.int) {
	defer func() {
		if err := recover(); err != nil {
			fmt.Printf("[crit] openssl: go_read_bio_read panic'd: %v\n", err)
			rc = -1
		}
	}()
	ptr := loadReadPtr(b)
...
func go_read_bio_ctrl(b *C.BIO, cmd C.int, arg1 C.long, arg2 unsafe.Pointer) (
	...
	defer func() {
		if err := recover(); err != nil {
			fmt.Printf("[crit] openssl: readBioCtrl panic'd: %v\n", err)
			rc = -1
		}
	}()
	switch cmd {
